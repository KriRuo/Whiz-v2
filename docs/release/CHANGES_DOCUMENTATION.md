...
## Repository Structure Reorganization (2025)

### Overview
The repository has been reorganized to follow Python best practices and improve maintainability. All files have been moved into organized directories while maintaining backward compatibility.

### Changes Made

#### Test Files Organization
- **Before**: 56 test files scattered in root directory
- **After**: 
  - `tests/unit/` - Unit tests (48 files)
  - `tests/integration/` - Integration tests (1 file)
  - `tests/verification/` - Verification scripts (2 files)
- **Impact**: All tests still work, imports updated automatically

#### Scripts Organization
- **Before**: 24+ scripts mixed in root directory
- **After**:
  - `scripts/launch/` - Launch scripts (4 files: .bat, .ps1, .vbs)
  - `scripts/build/` - Build scripts (10 files: .py, .bat, .sh)
  - `scripts/tools/` - Utility scripts (14 files: test runners, setup, etc.)
- **Impact**: Scripts automatically change to project root, so they work from new locations

#### Documentation Organization
- **Before**: 25 markdown files scattered in root
- **After**:
  - `docs/architecture/` - Architecture documentation (4 files)
  - `docs/guides/` - User guides (9 files)
  - `docs/release/` - Release documentation (4 files)
  - `docs/README.md` - Documentation index
- **Impact**: All cross-references updated, easier navigation

#### Assets Organization
- **Before**: 9 asset files in root
- **After**:
  - `assets/images/icons/` - Icon files (5 .ico files)
  - `assets/images/` - Image files (4 .png files)
- **Impact**: Better organization, no functional changes

### Backward Compatibility

All scripts and tests maintain backward compatibility:
- **Scripts**: Automatically change to project root directory before execution
- **Tests**: Import paths updated to work from new locations
- **Documentation**: Cross-references updated to new paths

### Updated File Paths

Users should now reference:
- Launch scripts: `scripts/launch/launch-whiz.bat` (instead of `launch-whiz.bat`)
- Build scripts: `scripts/build/build-executable.py` (instead of `build-executable.py`)
- Test runner: `scripts/tools/run_tests.py` (instead of `run_tests.py`)
- Setup script: `scripts/tools/setup_and_run.py` (instead of `setup_and_run.py`)

### Benefits

1. **Better Organization**: Clear separation of concerns
2. **Easier Navigation**: Find files quickly by category
3. **Scalability**: Easy to add new tests, scripts, or docs
...
- `scripts/tools/setup_and_run.py` - Updated Python version check
- `scripts/tools/install-and-run-macos.sh` - Updated Python version check
- `scripts/tools/install-and-run-linux.sh` - Updated Python version check
- `docs/guides/MACOS_TESTING_GUIDE.md` - Updated Python version
- `docs/release/PLATFORM_SUPPORT.md` - Updated Python version references
